...
func (cq *ConnectionQuery) QueryOwner() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, connection.OwnerTable, connection.OwnerColumn),
		)
...
func (cq *ConnectionQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (cq *ConnectionQuery) WithOwner(opts ...func(*UserQuery)) *ConnectionQuery {
	query := (&UserClient{config: cq.config}).Query()
...
//		GroupBy(connection.FieldLocalID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Connection.Query().
//		Select(connection.FieldLocalID).
//		Scan(ctx, &v)
...
func (cq *ConnectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Connection, error) {
	...
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *Connection, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConnectionQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Connection, init func(*Connection), assign func(*Connection, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Connection)
	for i := range nodes {
		if nodes[i].user_connections == nil {
			continue
		}
		fk := *nodes[i].user_connections
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_connections" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
