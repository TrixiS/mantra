...
func (uq *UserQuery) QueryConnections() *ConnectionQuery {
	query := (&ConnectionClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withConnections; query != nil {
		if err := uq.loadConnections(ctx, query, nodes,
			func(n *User) { n.Edges.Connections = []*Connection{} },
			func(n *User, e *Connection) { n.Edges.Connections = append(n.Edges.Connections, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadConnections(ctx context.Context, query *ConnectionQuery, nodes []*User, init func(*User), assign func(*User, *Connection)) error {
	...
	for _, n := range neighbors {
		fk := n.user_connections
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_connections" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_connections" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
